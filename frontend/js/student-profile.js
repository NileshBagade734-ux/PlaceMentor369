...
document.getElementById("buildResumeBtn")?.addEventListener("click", () => {
    const name = fullNameInput.value.trim() || "N/A";
    const roll = rollInput.value.trim() || "N/A";
    const branch = branchSelect.options[branchSelect.selectedIndex]?.text || "N/A";
    const cgpa = cgpaInput.value || "N/A";
    const skillsList = skills.length
        ? skills.map(s => `<span style="background:#dbeafe;color:#1d4ed8;padding:4px 10px;border-radius:20px;font-size:13px;margin:3px;display:inline-block">${s.name} <small>(${s.level})</small></span>`).join("")
    ...
    const resumeHTML = `
        ...
        <head>
            ...
            <style>
                body { font-family: 'Segoe UI', sans-serif; max-width: 800px; margin: 40px auto; color: #1e293b; padding: 0 20px; }
                h1 { font-size: 28px; color: #2563eb; margin-bottom: 4px; }
                .subtitle { color: #64748b; font-size: 14px; margin-bottom: 24px; }
                .section { margin-bottom: 24px; border-top: 2px solid #e2e8f0; padding-top: 16px; }
                .section h2 { font-size: 14px; text-transform: uppercase; letter-spacing: 1px; color: #64748b; margin-bottom: 10px; }
                .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
                .info-item label { font-size: 12px; color: #94a3b8; display: block; }
                .info-item span { font-size: 15px; font-weight: 600; }
                .skills-wrap { margin-top: 8px; }
                @media print { body { margin: 20px; } }
            </style>
        </head>
        <body>
            <h1>${name}</h1>
            <p class="subtitle">Roll No: ${roll} &nbsp;|&nbsp; ${branch} &nbsp;|&nbsp; CGPA: ${cgpa}</p>
            ...
            <div class="section">
                <h2>Skills</h2>
                <div class="skills-wrap">${skillsList}</div>
            </div>

            <div class="section">
                <h2>Generated By</h2>
                <p style="color:#94a3b8;font-size:13px">PlacementorAI — ${new Date().toLocaleDateString()}</p>
            </div>
        </body>
        </html>
    `;
