...
export const applyJob = async (req, res) => {
  try {
    ...
    const [studentProfile, job] = await Promise.all([
...
export const getLearningPaths = async (req, res) => {
  try {
    ...
    const topMissingSkills = aggregateAnalysis.topMissingSkills.map((item) => item.skill);
...
export const uploadResume = async (req, res) => {
  try {
    ...
    if (parsedResume.error && !parsedResume.text) {
      return res.status(400).json({ message: parsedResume.error });
    }
    ...
    student.resumeParseStatus = parsedResume.flagged ? "flagged" : "success";
    if (parsedResume.reviewEntry) {
      student.resumeReviewQueue = parsedResume.reviewEntry;
    }
